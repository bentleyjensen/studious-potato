...
*     Getting my coordinates to behave nicely translating from the column
*     letter to the index integer.
*
******************************************************************************/
...
void edit(int board[9][9])
{
   ...
   else
   {
      ...
      if (isdigit(value))
      {
         // It's already a valid numeral, store as int
         iValue = atoi(&value);

         // This will set only if the char passes isdigit
         inRange = true;
      };
...
bool validValue(int board[9][9], int row, int col, int iValue)
{
   ...
   if (valid)
   {
      ...
      // The latter two ought to occur simultaneously
   ...
   // Cool things that could easily be accomplished
   // Not enabled or written so the program passes testBed:
   // FEATURE:
   // report which square has the repeated value in it
   // ENHANCEMENT:
